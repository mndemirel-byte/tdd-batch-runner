# Web uygulamasi ornek projesi ([project-name]) icin gercek kosudan alinmis config
# ---------------------------------------------------------------------------
...
# --- Dal modeli -------------------------------------------------------------
# Bu repo'nun varsayilan dali master (git branch --show-current).
...
# package.json scripts'inden okundu:
#   test = vitest run | lint = eslint | build = next build
# TYPECHECK icin `npm run typecheck` (= tsc --noEmit) de var; ama issue kabul
# kriterlerinin 13'u birebir `npx tsc --noEmit` yaziyor ve G14b TAM ESLESME
# aradigi icin kriterdeki yazim kullanildi.
TEST_CMD="${TEST_CMD:-npm test}"
LINT_CMD="${LINT_CMD:-npm run lint}"
TYPECHECK_CMD="${TYPECHECK_CMD:-npx tsc --noEmit}"
BUILD_CMD="${BUILD_CMD:-npm run build}"               # final kapida kullanilir
E2E_CMD="${E2E_CMD:-}"                                # e2e script'i yok -> G12 atlanir
...
# Repoda dogrulandi: 71 test dosyasinin tamami `*.test.ts(x)`.
# `.spec.` ve `__tests__/` hic kullanilmiyor (0 eslesme) -> desen daraltildi.
TEST_FILE_RE="${TEST_FILE_RE:-(\.test\.)}"
...
# bypassPermissions: kosu insana hic sormaz. acceptEdits'te Bash yine sorar ve
# headless kosuda soru = izin reddi = sinif A (retry YOK) — issue 01'in ilk
# turunda tam olarak bu oldu.
#
# Savunulabilir olmasinin sarti u"c katmanin birlikte durmasi (dokuman §5.3):
#   1. Her issue kendi git worktree'sinde  (run-issues.sh)
#   2. G0 anti-tamper, repo DISINDAKI salt-okunur kopyadan calisir
#   3. .claude/settings.json deny kurallari — bypassPermissions bunlari
#      KALDIRMAZ; curl/wget/git push/npm publish kapali kalir
PERMISSION_MODE="${PERMISSION_MODE:-bypassPermissions}"

# Ajanin kullanabilecegi araclar. Dar tutmak G0'in tamamlayicisidir.
# Skill sart: kosucu prompt olarak "/implement-all ..." veriyor. Model bunu
# bazen dogrudan genisletiyor (deneme 1), bazen Skill aracini cagiriyor
# (deneme 2). Ikinci durumda Skill listede yoksa izin reddi olusur ve kosu
# sinif A ile durur -- retry de yapilmaz.
ALLOWED_TOOLS="${ALLOWED_TOOLS:-Bash,Read,Edit,Write,Glob,Grep,Task,Skill}"
